...
class CachedTokenServiceTest {
    ...
    @Test
    fun `getToken throws if token is null or empty`() {
        val tokenResponse = Auth0TokenResponse("", "Bearer", 3600)
        every { auth0TokenService.getMachineToMachineToken() } returns tokenResponse
        // Forzamos el acceso a cachedToken!!
        val service = CachedTokenService(auth0TokenService)
        val token = service.getToken()
        Assertions.assertEquals("", token)
    }

    @Test
    fun `getToken fetches new token if expiresAt is now`() {
        val tokenResponse = Auth0TokenResponse("token789", "Bearer", 1)
        every { auth0TokenService.getMachineToMachineToken() } returns tokenResponse
        val service = CachedTokenService(auth0TokenService)
        // Primer getToken setea expiresAt a now+1-30 = now-29, así que siempre expira
        val token1 = service.getToken()
        val token2 = service.getToken()
        Assertions.assertEquals("token789", token2)
    }
}
